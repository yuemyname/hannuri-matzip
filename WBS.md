...
- **시크릿 유출 없음 확인.** 카나리 값으로 빌드해 `.next/static` 을 뒤져 0건.
  ...
  검사 방법 자체가 유효하다는 뜻이다.
- ⚠️ **실물 호출 미검증.** 작업 환경이 `*.ntruss.com` 을 막는다. 엔드포인트
  `/search/v1/local` 은 가이드의 news 예시(`/v1/search/news.json` → `/search/v1/news`)
  에서 규칙을 옮겨 적은 것이라 실제로 한 번 불러 봐야 한다. `mapx` 스케일도 마찬가지 —
  판정 로직이 셋 다 받아내지만 실제로 어느 쪽인지는 응답을 봐야 안다.
