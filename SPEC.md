...
- 컨트롤은 **지도 위에 떠 있다** (하단에 붙은 바가 아니다). 바로 두면 그만큼
  ...
  - **[+] 의 맨 아래는 후원(ko-fi)이다** (2026-08-12 요청). 이 앱에서 밖으로
    나가는 유일한 링크라 맨 아래에 둔다 — 위 다섯은 전부 앱 안에서 할 일이다.
    - ko-fi 가 주는 버튼 그림(`storage.ko-fi.com/cdn/kofi3.png`)을 **그대로**
      쓴다. 우리 칩 모양으로 다시 그리지 않는다: 남의 서비스로 넘어가는 링크는
      그쪽 상표로 보이는 편이 눌러도 되는지 판단하기 쉽다. 그래서 여기만
      테두리·그림자가 없다 (그림이 이미 버튼 모양이다).
    - 높이는 `h-9`. ko-fi 가 지정한 36px 이자 위 항목들의 높이라, 오른쪽 변이
      한 줄로 떨어진다. 가로는 그림 비율대로 둔다 — 못 박으면 찌그러진다.
    - `next/image` 를 안 쓴다. 외부 도메인 등록이 필요한데 ko-fi 가 크기까지
      정해 놓은 36px 짜리라 최적화할 여지가 없고, 최적화기를 거치면 ko-fi 가
      안 닿을 때 500 이 뜬다 — `img` 면 대체 글자로 내려앉는다.
    - `target="_blank"` 에는 `rel="noreferrer"` 를 같이 단다. 안 달면 새 탭에서
      `window.opener` 로 이 탭의 주소를 바꿀 수 있다.
    - 대체 글자는 「커피 한 잔 사주기 (새 창)」. ko-fi 원본 alt 는 영문이고,
      새 탭이 열린다는 사실은 눌러 보기 전에 알아야 한다.
...
- **[뽑기]** → 서버 응답 먼저 수신 → **룰렛** → 결과 카드
  ...
  **세게 돌린다** (한 바퀴 0.4초, 감속하며 5바퀴 이상). 처음엔 느슨하게 한 바퀴
  남짓만 더 돌게 했는데, 감속 뒷부분이 너무 느려 "벌써 섰나" 로 보였다 — 시간은
  다 흘렀는데 눈에는 짧게 느껴진 것이다. 눈에 보이는 회전이 3.8초 이어진다.
  ...
  - 감속이 끝나도 **0.7초는 멈춘 채로 서 있다.** 예전에는 서는 순간 결과 카드가
    판을 덮어서 어디에 섰는지 볼 틈이 없었고, 그래서 "돌기는 도나" 로 읽혔다.
  - `prefers-reduced-motion` 이면 **돌리지 않고 답 위에 세워** 0.7초 보여준다.
  ...
  - 판은 **캔버스로 그린다** (2026-08-11 재작업). `conic-gradient` + 가운데 창은
    ...
    되는데, 그건 CSS 로는 못 한다.
    - 그림은 **한 번만** 그린다. 회전은 CSS 가 요소째로 돌리므로 그림은 안 바뀐다.
    - **회전은 CSS 가 한다** (2026-08-11 최종. 자바스크립트로 두 번 시도했다가
      둘 다 실기기에서 끊겼다). 대기 중에는 키프레임(`--animate-pick-spin`),
      답이 오면 `transform` 트랜지션 한 번이다. rAF 로 매 프레임 각도를 옮기면
      메인 스레드에 얹히는데, 이 화면 뒤에는 지도가 살아 있고 조회도 돈다 —
      그래서 끊겼다. 키프레임·트랜지션은 합성 스레드가 돌려서 안 끊긴다.
      **도는 동안 자바스크립트는 한 줄도 안 돈다.**
      - 키프레임 → 트랜지션으로 갈아탈 때는 **지금 각도를 고정하고 리플로우를
        한 번 일으킨 뒤** 목표를 건다. 애니메이션이 그리는 값은 트랜지션의
        시작점이 아니라서, 바로 걸면 0도에서 다시 출발한다.
    - **연출 시작을 렌더 중에 세운다.** 응답을 받은 뒤 이펙트에서 켜면,
      `isPending` 은 이미 false 인데 연출 플래그는 아직 false 인 한 렌더가
      생겨서 룰렛이 통째로 사라졌다 다시 붙는다. 새로 붙은 판은 인라인 스타일이
      없으니 등속 회전이 꺼진 채로 시작하고, 응답이 빠른 평소에는 판이 1초간
      가만히 서 있다가 갑자기 감속만 하고 멎는다.
    - 감속 곡선은 `--ease-roulette` — **마찰로 서는 곡선**(등감속)에 맞춘 값이다.
      `--ease-out-soft` 는 절반 시점에 이미 96% 를 가 버려서 나머지 절반이
      기어가는 것처럼 보였다.
    - 캔버스에 `rounded-full` 을 안 건다 — 판이 이미 원이라 마스크가 하는 일이
      없는데 도는 내내 합성 비용만 낸다. 백업 저장소는 2배까지만 키운다.
    - 칸은 **브랜드색(`--primary`)과 흰색(`--background`)을 번갈아** 칠한다
      (2026-08-11 최종). 처음엔 카테고리 색 일곱을 그대로 썼다가 탁한 파이가
      됐고, 파스텔로 낮춰도 서로 뭉쳐 보였다. 종류마다 색을 주려면 일곱 색이
      한 화면에서 서로 싸우는데, **칸을 가르는 건 원래 색이 아니라 박아 넣은
      이름**이다. 두 색이면 칸 경계만 또렷하고 색끼리 싸울 일이 없다.
      - 글자색은 칸을 따라간다 — 브랜드 칸은 `--primary-foreground`,
        흰 칸은 `--foreground`. 한 색으로 통일하면 한쪽이 반드시 안 읽힌다.
      - 칸 수가 홀수면 마지막 칸과 첫 칸이 **같은 색으로 맞붙는다.**
        그 이음매에만 금을 긋는다 (브랜드끼리면 `--primary-foreground`,
        흰색끼리면 테두리색). 모든 칸에 금을 그으면 색이 이미 갈라 놓은 자리에
        선이 겹쳐 지저분하다.
