...
- 판이 끝나면 남은 것도 **잠그고 물러난다**(muted). 흰 바탕 그대로면 아직 누를 수
  ...
  4.5:1 아래로 떨어진다.
- 색만으로 알리지 않는다. 「꽝」·「밥값」이 **글자로** 박히고, 결과 줄은 `aria-live`
  가 "N번이 내는 걸로" 로 읽는다. 칸 하나하나의 이름도 「3번 — 밥값」 식이다.
- 아래 버튼은 **[다시 접기]** / **[다시 놓기]** 다. 뽑는 건 제비를 누르거나 줄을
  고르는 것이라, "다시 뽑기" 면 이 버튼을 눌러야 뽑히는 줄 안다.

**제비뽑기**
- **5열 고정**이다. 인원에 따라 열을 바꾸면 같은 화면이 매번 다른 모양이 되고,
  360px 에서 스무 장이 한 줄에 못 들어간다.
- 펼치는 연출은 `--animate-slip-open` (0.34초). 가운데에서 종이가 모로 서는 순간에
  앞뒤를 바꾼다 — 0에 바꾸면 펼치기도 전에 답이 보인다.
...
- 줄 긋는 연출은 `--animate-ladder-trace` (0.75초). `pathLength="1"` 을 걸어
  줄 길이와 무관하게 같은 속도로 그어지게 한다.
