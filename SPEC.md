...
5. 가중치 랜덤 — **네 항목의 곱** (2026-08-09 개정, 이전은 `(avg_rating+1)^1.5`)

   ```
   weight = score^2  ×  mood  ×  price_fit  ×  near
   ```
   ...
   - **`near` — 가까울수록 조금 유리.** `1 + k×(1 − distance/radius)`.
     `k` 는 시간대마다 다르다 (2026-08-09): **점심 0.45 / 저녁 0.1.** 점심은
     한 시간 안에 다녀와야 해서 100m 가 진짜 차이고, 저녁은 시간이 있어 거의
     안 본다. 그래도 상한을 낮게 둔다 — 거리로 결정되면 추천이 아니라 정렬이다.
   - **`price_fit` — 태그가 없어도 시간대를 가르는 축** (2026-08-09).
     `1 ± 0.15×(price_range − 2.5)`, 저녁은 비싼 쪽 / 점심은 싼 쪽. 양 끝 차이
     약 1.58배로, 태그(최대 2.7배)보다 확실히 약하다 — 추측이 사람의 기록을
     이기면 안 된다. 가격대가 비어 있으면 1.0.

6. **종류별로 무게를 고른다** (2026-08-12 요청: "한식이 너무 많이 나와").
   각 곳의 weight 를 **제 종류의 weight 합**으로 나눈다. 그러면 종류 하나가
   통째로 무게 1이 되어, 이렇게 뽑은 것과 같아진다:

   ```
   1) 후보에 남은 종류 중 하나를 고른다 — 종류마다 같은 확률
   2) 그 종류 안에서 위 weight 로 한 곳을 고른다
   ```

   전에는 곳 단위로만 뽑아서, 근처에 한식이 15곳·카페가 3곳이면 한식이 그대로
   5배 자주 나왔다. 계산은 맞았지만 화면의 룰렛이 칸을 똑같은 크기로 그려서
   "종류는 공평하다" 고 약속하는 것처럼 읽혔다 — 보이는 것과 실제를 맞춘다.

   뒤집으면 이렇다: **곳이 하나뿐인 종류도 열다섯 곳짜리와 같은 확률이다.**
   점메추의 목적이 "가장 그럴듯한 한 곳" 이 아니라 "오늘 뭐 먹지" 를 끊어 주는
   것이라 골고루 도는 쪽을 골랐다. **종류 안의 순위는 하나도 안 바뀐다** —
   평점·태그·가격대·거리는 그대로 먹는다. 카테고리를 골라 뒀으면 그 안에서만
   공평하다 (「한식·중식」만 골랐으면 둘이 반반).

7. `order by -ln(1 - random()) / weight limit 1` (가중 샘플링 표준 트릭)
   - `random()` 이 아니라 `1 - random()` 이다. `random()` 은 0 을 낼 수 있고
     `ln(0)` 은 예외라, 아주 드물게 점메추가 통째로 터진다. 분포는 같다.
...
> **화면 제목은 가운데다** (2026-08-12 요청). 모달 머리(`Modal`)와 풀페이지
> 머리(`PageShell`) 둘 다, 양옆에 **같은 몫의 칸**을 두고 가운데 칸에 제목을
> 넣는다 — [닫기]·[뒤로] 폭만큼만 비워 두면 글자 길이에 따라 제목이 좌우로
> 흔들린다.
...
- **반경은 30·50·100·200·500m·1km** (2026-08-12 요청, 상한을 200m 에서 올렸다).
  원래 상한은 "100m 넘게 걸어가서 먹는 일이 없다" 로 잡은 것인데, 그건 맛집이
  촘촘히 등록된 뒤의 이야기였다. 200m 안에 몇 곳뿐이면 뽑을 게 없다는 말만 나오고
  종류도 한둘로 쏠린다. 도보 12분쯤인 1km 까지 열어 두고, 넓히는 건 사용자가 고른다.
  - 1000m 는 「1km」로 적는다 (`radiusLabel`). 네 자리 미터는 읽는 데 한 박자 걸린다.
  - 여섯 칸이라 360px 에서는 **줄이 바뀐다.** 가로 스크롤로 두면 마지막 칸이
...
- **[뽑기]** → 서버 응답 먼저 수신 → **룰렛** → 결과 카드
  ...
  **세게 돌린다** (한 바퀴 0.4초, 감속하며 5바퀴 이상). 처음엔 느슨하게 한 바퀴
  ...
  - 판에 올리는 것은 **지금 나올 수 있는 것**이다 (2026-08-12 정정). 전에는
    조건과 상관없이 늘 전체 종류를 그려서, 「한식」만 골라 놔도 일곱 칸이
    돌았다 — 나올 수 없는 종류가 나올 것처럼 보이는 것은 판이 조건을 배신하는
    것이다. 무엇을 올릴지는 고른 종류 수가 정한다:

    | 고른 종류 | 칸에 올리는 것 |
    |---|---|
    | 0개 | `useCategories()` 전체 |
    | 1개 | **그 종류의 가게 이름** (반경 안, 가까운 순 10곳까지) |
    | 2개 이상 | 고른 종류들 |

    - **하나만 골랐을 때 가게 이름을 쓰는 이유**: 그 조건에서 실제로 갈리는
      것은 종류가 아니라 가게다. 「한식」 한 칸짜리 판은 뽑는 그림이 안 된다.
      이때만 `restaurants_within` 을 한 번 더 부른다 (반경·종류가 점메추와
      같은 값이고, 가격대는 클라이언트에서 한 번 더 거른다).
      명단이 아니라 그림이라 10곳에서 자른다 — 서른 칸은 안 읽힌다.
    - 칸 순서는 **고른 차례가 아니라 목록 순서**다. 칩을 누른 차례대로 세우면
      같은 조건인데 판이 달라 보인다.
    - 뽑힌 것이 칸에 없으면(목록이 낡았거나, 「최근 간 곳 빼기」로 클라이언트가
      모르는 후보가 나왔거나) 칸을 하나 덧붙인다 — 멈출 칸이 없으면 엉뚱한
      칸에 선다.
    - **후보가 하나뿐이면 판을 안 그린다.** 칸이 하나인 룰렛은 돌 이유가 없고,
      돌면 오히려 고장으로 보인다. 대신 그 이름을 크게 남긴다 — 판만 지우면
      「고르는 중」 넉 자만 남아 무엇을 고르는 중인지가 사라진다.
  - **칸이 적으면 같은 이름을 되풀이해 여덟 칸쯤 만든다** (2026-08-12 요청).
    둘이면 반반, 셋이면 세 조각 — 그건 룰렛이 아니라 파이 차트다.
    ...
    - 한 바퀴를 통째로 되풀이해서 칸이 **번갈아** 놓인다 (A·B·A·B…).
      붙여 놓으면(A·A·B·B) 반반짜리 판과 다를 바 없다.
    - 여섯 이상은 안 늘린다. 이미 판으로 보이고, 더 늘리면 이름이 안 읽힌다.
    - 같은 이름이 여럿이면 어디에 설지는 **누른 시각**이 고른다. `Math.random()`
      을 쓰면 리렌더마다 목표가 움직인다 — 도는 도중에 설 자리가 바뀐다는 뜻이다.
      어느 칸에 서든 이름이 같으니 **결과와는 무관하다.**
  ...
  - 판은 **캔버스로 그린다** (2026-08-11 재작업). `conic-gradient` + 가운데 창은
    ...
    - 이름이 놓이는 폭은 **반지름이 아니라 그 자리에서 칸이 벌어진 폭**이다
      (2026-08-12). 이름은 칸을 가로지르는 방향으로 놓이는데, 부채꼴의 두 변은
      가운데에서 뻗어 나가므로 반지름 r 자리의 반폭이 `r·tan(칸각/2)` 다.
      안 들어가면 줄이고 `…` 를 붙인다. 예전엔 이 계산 없이 반지름의 72% 를
      그대로 썼다 — 종류 이름은 두세 자라 안 걸렸는데, 가게 이름을 올리자마자
      옆 칸을 파고들었다.
...
### 4.6 `/bill` — 밥값 내기 (2026-08-09 추가)

**제비뽑기 / 사다리 중에 골라서 한다** (2026-08-12 요청, 숫자 굴리기에서 교체).

인원수만 넣으면 그만큼 제비가 깔리거나 사다리가 놓이고, **한 사람씩 하나를
고른다.** 「밥값」이 나오면 그 판이 끝난다.

- 예전에는 숫자가 드르륵 굴러가다 하나에 멈췄다. 결과는 같지만 **뽑는 사람이
  없었다** — 버튼을 누른 한 사람이 답을 받아 읽어 주는 그림이라, 여럿이 둘러앉아
  하는 일에는 안 맞았다. 지금은 **고르는 차례가 곧 사람 차례**다.
- **두 놀이가 같은 뼈대를 쓴다.** 인원 / 밥값을 낼 번호(`payer`) / 이미 열어 본
  번호(`opened`) 까지는 같고, 다른 것은 그림과 여는 방법뿐이다. 결과 줄과 다시
  하기 버튼도 하나다. 놀이마다 화면을 통째로 따로 만들면 "밥값 내기" 가 둘이
  되고, 한쪽만 고쳐 놓는 일이 생긴다.
- 놀이를 바꾸면 판을 새로 깐다. 결과는 어느 쪽이나 같으니 **취향 문제**고,
  그래서 고르개를 인원보다 위에 둔다.
- **이름을 안 받는다.** 이 앱에는 신원이 없고(§2.4) 매번 이름을 치는 건 점심시간에
  할 일이 아니다. 번호만 있으면 번호를 나눠 갖든 아무거나 집든 둘 다
  굴러간다. "번호는 서로 정해 주세요. 앱은 누가 몇 번인지 몰라요" 를 화면에 적는다.
- **당첨은 판이 시작될 때 이미 정해져 있다.** 여는 순간에 정하면 마지막 하나의
  확률이 1이 되거나(남은 것 중에서 다시 굴리면) 앞에서는 아예 안 나온다. 종이
  제비도 사다리도 **놓는 순간** 이미 정해져 있다 — 여는 것은 확인이지 추첨이 아니다.
- **뽑기를 클라이언트에서 한다.** "추천 결과는 서버가 결정한다"(§3.2, CLAUDE.md)는
  여기 안 걸린다. 그 규칙이 있는 이유는 추천의 후보 풀이 서버에 있기 때문이다
  (RLS·최근 방문·평점 가중). 여기 입력은 숫자 하나뿐이라 서버가 아는 게 없다 —
  왕복만 늘고 오프라인에서 못 쓰게 된다.
- 인원은 2~20명. **인원을 바꾸면 판을 다시 깐다** ("4명 중 5번" 이 남으면 안 된다).
- **몇 명이 낼지도 정한다** (2026-08-12 요청). 회식비를 둘이 나눠 내는 날이 있다.
  1명부터 **인원보다 하나 적은 수**까지 — 전원이 당첨이면 뽑을 것이 없다.
  사람을 줄이면 당첨자 수도 따라 준다 ("3명 중 4명이 낸다" 는 없다).
  다 찾아야 판이 끝나고, 결과 줄은 "3번, 5번이 내는 걸로" 로 전부 적는다.
- 판이 끝나면 남은 것도 **잠그고 물러난다**(muted). 흰 바탕 그대로면 아직 누를 수
  있는 것처럼 보이는데 눌러도 안 열린다. 투명도는 안 씌운다 — 씌우면 번호 대비가
  4.5:1 아래로 떨어진다.
- 열린 칸에 적는 말은 **「냐미」 / 「잘먹었습니다」** 다 (2026-08-12 요청,
  꽝·밥값에서 교체). 두 놀이가 같은 말을 써야 한 화면으로 읽히므로 상수 하나로 둔다.
- 색만으로 알리지 않는다. 두 말이 **글자로** 박히고, 결과 줄은 `aria-live` 가
  "N번이 내는 걸로" 로 읽는다. 칸 하나하나의 이름도 「3번 — 잘먹었습니다」 식이다.
- 아래 버튼은 **[다시 접기]** / **[다시 놓기]** 다. 뽑는 건 제비를 누르거나 줄을
  고르는 것이라, "다시 뽑기" 면 이 버튼을 눌러야 뽑히는 줄 안다.

**제비뽑기**
- **4열 고정**이다. 인원에 따라 열을 바꾸면 같은 화면이 매번 다른 모양이 되고,
  360px 에서 스무 장이 한 줄에 못 들어간다. 다섯이던 것을 넷으로 줄인 것은
  「잘먹었습니다」가 들어갈 폭 때문이다 — 긴 말은 `text-balance` 로 두 줄에
  고르게 나눈다 (그냥 두면 「잘먹었습니 / 다」로 끊긴다).
- 펼치는 연출은 `--animate-slip-open` (0.34초). 가운데에서 종이가 모로 서는 순간에
  앞뒤를 바꾼다 — 0에 바꾸면 펼치기도 전에 답이 보인다. **등속이라야 한다** —
  `--ease-out-soft` 는 절반 시점에 이미 96% 라 종이가 도로 펴진 뒤에 글자가 바뀐다.
- **[한 번에 펼치기]** (2026-08-12 요청). 둘이서 눈치만 보다 끝내고 싶은 날이
  있다 — 한 장씩은 기본값이지 규칙이 아니다.
  - 동시에 확 펴지 않고 **왼쪽부터 물결로** 편다 (한 장당 0.17초). 스무 장이
    한 프레임에 뒤집히면 어느 장이 당첨인지 눈이 못 쫓는다.
    `prefers-reduced-motion` 이면 물결 없이 한 번에 다 편다.
  - 당첨을 다 찾아도 멈추지 않고 **끝까지 편다.** "다 펼치기" 를 눌렀는데 몇 장이
    접힌 채 남으면 그게 더 이상하다.
  - 몇 장을 손으로 연 뒤에 눌러도 **나머지 전부**가 펴진다. 판이 끝났으면 펼 것이
    없으니 버튼도 없다. 펼치는 중에는 손으로 못 열고, [다시 접기] 는 남은 타이머를
    걷어 낸다 — 안 걷으면 새 판을 이전 물결이 마저 열어 버린다.
  - 사다리에는 없다. 줄은 한 줄씩 따라 내려가는 게 놀이의 전부라, 한 번에 그으면
    남는 게 없다.

**사다리**
- 위 번호를 하나 고르면 줄이 그어지며 내려가고, 도착한 아래 칸이 열린다.
- 아래 칸은 **닿기 전에는 「?」** 다. 다 보여 주면 눈으로 따라가서 답을 미리 알 수
  있고, 그러면 남은 사람들이 고를 이유가 없어진다.
- **위와 아래는 일대일이다.** 한 층에서 이웃한 두 자리에 가로줄을 같이 놓지 않는다 —
  붙여 놓으면 한 층에서 두 칸을 건너뛰는데, 그건 사다리가 아니라 그냥 뒤섞기고
  눈으로 따라갈 수도 없다. (검사: 탄 사람 수 == 열린 아래 칸 수)
- 그림은 **SVG** 다. 선 하나씩이라 캔버스를 쓸 이유가 없고, 색도 클래스
  (`stroke-border`)로 줄 수 있어 토큰이 코드로 새지 않는다. 늘어난 뷰박스에서
  선이 같이 굵어지지 않도록 `vector-effect="non-scaling-stroke"` 를 건다.
- 지나간 줄은 판에 **남는다** — 누가 어디로 갔는지가 쌓여야 "다음은 어디" 를 서로
  보고 정한다. 다만 한 톤 죽이고(`stroke-muted-foreground`), **밥값 줄은 맨 나중에
  그린다**: 먼저 그리면 나중에 탄 줄이 그 위를 덮어 답이 중간에 끊겨 보인다.
- 스무 명이면 세로줄이 스무 개다. 폭을 다 나눠 쓰면 손가락이 안 닿으니 세로줄
  하나에 최소 너비를 주고 **판만 가로로 민다** (문서는 안 넘친다).
- 줄 긋는 연출은 `--animate-ladder-trace` (0.75초, 등속). **줄 위에 덮개를
  씌웠다가 위에서부터 걷는다**(`clip-path: inset()`). 두 가지가 안 됐다:
  - `stroke-dasharray` — 판을 가로로 늘려 놓은 탓에 점선 길이가 어긋나서, 다
    그어진 줄이 중간중간 끊겨 보였다.
  - `<clipPath>` 안의 도형을 움직이기 — 그 안의 요소는 그려지지 않아서 CSS
    애니메이션이 아예 시작하지 않는다.

**둘 다** — `prefers-reduced-motion` 이면 **기다리지 않고 바로 연다**: 전역
미디어쿼리는 CSS 만 줄이고 자바스크립트 타이머는 못 줄이므로, 그냥 두면 연출은
없는데 답만 늦게 나오는 굼뜬 화면이 된다.
