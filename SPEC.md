...
- 목록은 두 칸이다. 위가 **내 지도에 있는 곳(최대 4)**, 아래가 **네이버에서 찾은
  아직 없는 곳**. 네이버 것만 늘어놓으면 "여긴 아무것도 없나" 로 보이고 방금 담은
  것이 어디로 갔는지도 모른다. 다만 이 화면의 일은 채우는 것이라 **우리 것은
  맥락일 뿐**이라 넷까지만 얹는다 — 등록분이 적으면 그만큼 네이버 쪽이 길어진다.
  넷을 넘겨 감춘 개수는 화면에 적는다 (조용히 자르지 않는다).
- **거르기는 자르기 전 전체로 한다.** 넷만 보고 거르면 다섯 번째로 가까운 등록분이
  네이버 후보로 다시 떠서, 눌러도 인덱스가 막는다.
- 기본 질의는 `{지역명} 맛집`. 종류 칩을 누르면 `{지역명} {종류}` 로 좁힌다.
  질의는 §4.4 와 같은 3중 그물(동·구·맨질의)을 그대로 탄다.
...
- 담을 때 채우는 값: 이름·좌표·주소·전화·플레이스 링크·종류(네이버 분류로 추정).
  가격대·메모·상황 태그·예약은 **비운다.** 아는 사람이 상세에서 채우면 된다 —
