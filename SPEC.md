...
- **[뽑기]** → 서버 응답 먼저 수신 → **룰렛** → 결과 카드
  ...
  **세게 돌린다** (한 바퀴 0.4초, 감속하며 5바퀴 이상). 처음엔 느슨하게 한 바퀴
  ...
  - 칸은 **지금 나올 수 있는 종류**다 (2026-08-12 정정). 카테고리를 골랐으면
    고른 것만, 안 골랐으면 `useCategories()` 전체다. 전에는 조건과 상관없이 늘
    전체를 그려서, 「한식」만 골라 놔도 일곱 칸이 돌았다 — 나올 수 없는 종류가
    나올 것처럼 보이는 것은 판이 조건을 배신하는 것이다.
    - 칸 순서는 **고른 차례가 아니라 목록 순서**다. 칩을 누른 차례대로 세우면
      같은 조건인데 판이 달라 보인다.
    - 뽑힌 종류가 칸에 없으면(목록이 낡았거나, 조건을 바꾸는 사이에 답이 온
      경우) 그 종류를 덧붙인다 — 멈출 칸이 없으면 엉뚱한 칸에 선다.
    - **한 종류만 골랐으면 판을 안 그린다.** 칸이 하나인 룰렛은 돌 이유가 없고,
      돌면 오히려 고장으로 보인다. 대신 고른 종류 이름을 크게 남긴다 — 판만
      지우면 「고르는 중」 넉 자만 남아 무엇을 고르는 중인지가 사라진다.
