...
**제비뽑기다** (2026-08-12 요청, 숫자 굴리기에서 교체).

인원수만 넣으면 접힌 제비가 그만큼 깔리고, **한 사람씩 한 장을 연다.**
편 제비에는 「꽝」이나 「밥값」이 적혀 있고, 「밥값」이 나오면 그 판이 끝난다.

- 예전에는 숫자가 드르륵 굴러가다 하나에 멈췄다. 결과는 같지만 **뽑는 사람이
  없었다** — 버튼을 누른 한 사람이 답을 받아 읽어 주는 그림이라, 여럿이 둘러앉아
  하는 일에는 안 맞았다. 지금은 **여는 차례가 곧 사람 차례**다.
- **이름을 안 받는다.** 이 앱에는 신원이 없고(§2.4) 매번 이름을 치는 건 점심시간에
  할 일이 아니다. 제비에 번호만 적어 두면 번호를 나눠 갖든 아무 장이나 집든 둘 다
  굴러간다. "번호는 서로 정해 주세요. 앱은 누가 몇 번인지 몰라요" 를 화면에 적는다.
- **당첨은 판이 시작될 때 이미 정해져 있다.** 여는 순간에 정하면 마지막 한 장의
  확률이 1이 되거나(남은 장 중에서 다시 굴리면) 앞에서는 아예 안 나온다. 종이 제비도
  접는 순간 이미 정해져 있다 — 여는 것은 확인이지 추첨이 아니다.
...
- 인원은 2~20명. **인원을 바꾸면 판을 다시 접는다** ("4명 중 5번" 이 남으면 안 된다).
- 제비판은 **5열 고정**이다. 인원에 따라 열을 바꾸면 같은 화면이 매번 다른 모양이
  되고, 360px 에서 스무 장이 한 줄에 못 들어간다.
- 판이 끝나면 남은 제비도 **잠그고 접힌 채로 물러난다**(muted). 흰 바탕 그대로면
  아직 누를 수 있는 것처럼 보이는데 눌러도 안 열린다. 투명도는 안 씌운다 —
  씌우면 번호 대비가 4.5:1 아래로 떨어진다.
- 색만으로 알리지 않는다. 「꽝」·「밥값」이 **글자로** 박히고, 결과 줄은 `aria-live`
  가 "N번이 내는 걸로" 로 읽는다. 제비 하나하나의 이름도 「3번 — 밥값」 식이다.
- 아래 버튼은 **[다시 접기]** 다. 뽑는 건 제비를 누르는 것이라, "다시 뽑기" 면
  이 버튼을 눌러야 뽑히는 줄 안다.
- 펼치는 연출은 `--animate-slip-open` (0.34초). 가운데에서 종이가 모로 서는 순간에
  앞뒤를 바꾼다 — 0에 바꾸면 펼치기도 전에 답이 보인다. `prefers-reduced-motion`
  이면 **기다리지 않고 바로 편다**: 전역 미디어쿼리는 CSS 만 줄이고 이 타이머는
  못 줄이므로, 그냥 두면 연출은 없는데 답만 0.17초 늦는 굼뜬 화면이 된다.
