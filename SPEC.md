...
- **조회 범위는 화면에 보이는 영역이다** (2026-08-09). 반경 토글과 반경 Circle 은
  ...
  - **마커는 한 화면에 100개까지** (`MAX_MARKERS`). 다 그리면 핀이 뭉개져서
    "많다" 말고는 아무것도 안 알려준다. 넘치면 **조용히 자르지 않고** 말한다:
    "너무 많아서 일부만 보여요 · 확대해 보세요". 잘렸는지는 상한보다 한 건 더
    받아서 안다 — 딱 상한만큼 받으면 "마침 100곳" 과 구분이 안 된다.
  - **줌 13 미만이면 아예 조회하지 않는다** (`MIN_QUERY_ZOOM`). 어차피 잘리고,
    잘린 100개는 쓸모가 없다. 그때는 마커도 걷고 "조금 확대하면 맛집이 보여요".
  - 클러스터링(겹친 마커를 숫자 원으로 합치기)은 아직 안 넣었다. 네이버 모듈은
    별도 JS 를 직접 호스팅해야 하는 의존성이고, 사내 수십 곳 규모에는 과하다.
    한 화면에 100개가 자주 걸리기 시작하면 그때가 도입 시점이다.

- **데스크톱도 같은 레이아웃.** 2단 분할을 없앴다.
- 카테고리 칩은 **복수 선택**이고, 고른 종류의 맛집만 마커로 남는다.
  별도 마커 필터가 있는 게 아니라 조회 조건이 바뀌는 것이다 — 마커는 항상
  조회 결과 배열을 그대로 그린다.
- 마커를 고르면 맛집 카드 1장이 지도 위에 뜬다. **리스트가 없으므로 이게
  지도에서 상세로 가는 유일한 길이다.** 없애지 말 것.
- 정렬은 없다. 리스트 순서를 정하는 장치였고, 마커에는 순서가 없다.
...
>
> 그래서 **행정동을 검색어에 붙인다.** 지도 SDK 를 `submodules=geocoder` 로 이미
> 싣고 있어서 `naver.maps.Service.reverseGeocode` 로 "광진구 구의동" 을 얻어
> `광진구 구의동 써브웨이` 로 보낸다. 키가 더 필요하지 않다.
> - 지역명을 붙여 0건이면 원래 질의로 한 번 더 찾는다 (체인점은 상호에 지역이
>   안 붙는다). 추가 호출은 0건일 때만이라 평소 쿼터는 그대로다.
> - 역지오코딩이 실패하거나 응답이 없으면(3초) 지역명 없이 찾는다. 등록을 막지 않는다.
> - 후보는 **현재 위치에서 가까운 순**으로 세우고 거리를 함께 적는다.
>   지역명을 붙여도 먼 데가 섞여 오기 때문이다.
>
