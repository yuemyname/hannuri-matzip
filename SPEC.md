...
- **[뽑기]** → 서버 응답 먼저 수신 → **룰렛** → 결과 카드
  ...
  **세게 돌린다** (한 바퀴 0.4초, 감속하며 5바퀴 이상). 처음엔 느슨하게 한 바퀴
  ...
  - 판은 **캔버스로 그린다** (2026-08-11 재작업). `conic-gradient` + 가운데 창은
    ...
    - 칸 색은 카테고리 색의 **파스텔 형제**(`--color-cat-*-pastel`)고 글자는
      어두운 잉크다. 칩 색은 흰 글자를 얹으려고 어둡게 잡은 것이라, 일곱을 한 판에
      모으면 탁한 파이가 된다. 색상(H)은 같아서 "이 색이 그 종류" 는 그대로다.
      파스텔은 서로 가까워지기 쉬워 색상을 일부러 벌려 놨다 — 「기타」는 칩에서는
      따뜻한 회색인데 판에서는 차가운 회색이다 (안 그러면 한식·카페와 뭉친다).
      그래도 칸을 가르는 건 색이 아니라 **박아 넣은 이름**이다.
